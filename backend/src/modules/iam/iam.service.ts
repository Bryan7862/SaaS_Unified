...
export class IamService implements OnModuleInit {
  ...
  private readonly STANDARD_PERMISSIONS = [
    ...
    { resource: "users", action: "update", description: "Editar usuarios" },
    {
      resource: "users",
      action: "delete",
      description: "Eliminar/Suspender usuarios",
    },
    ...
    // Billing/Subscriptions
    {
      resource: "billing",
      action: "read",
      description: "Ver facturación y planes",
    },
    {
      resource: "billing",
      action: "update",
      description: "Modificar suscripción",
    },
    ...
    // Trash (Papelera)
    {
      resource: "trash",
      action: "audit",
      description: "Ver historial de auditoría de papelera",
    },
    {
      resource: "trash",
      action: "hard_delete",
      description: "Eliminar permanentemente elementos",
    },
  ];
  ...
  async assignRole(
    ...
    if (actorId) {
      ...
      if (newRole.code === "OWNER") {
        if (!actorRole || actorRole.role.code !== "OWNER") {
          throw new BadRequestException(
            "Solo un Propietario (Owner) puede nombrar nuevos Owners.",
          );
        }
      }
    }
    ...
    if (
      ...
      existingAssignments[0].roleId === roleId
    ) {
    ...
    const isCurrentlyOwner = existingAssignments.some(
      (ur) => ur.role.code === "OWNER",
    );
    ...
    if (isCurrentlyOwner && newRole.code !== "OWNER") {
      ...
      if (ownerRole) {
        ...
        if (ownerCount <= 1) {
          throw new BadRequestException(
            "No puedes cambiar el rol del último PROPIETARIO (Owner). Asigna otro Owner primero.",
          );
        }
      }
    }
  ...
  async deleteRole(roleId: string): Promise<void> {
    ...
    if (["OWNER", "ADMIN", "MEMBER"].includes(role.code)) {
      throw new BadRequestException(
        "No puedes eliminar roles del sistema (OWNER, ADMIN, MEMBER).",
      );
    }
    ...
    if (usageCount > 0) {
      throw new BadRequestException(
        `No puedes eliminar este rol porque está asignado a ${usageCount} usuarios. Reasigna esos usuarios primero.`,
      );
    }
