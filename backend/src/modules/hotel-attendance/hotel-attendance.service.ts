...
export class HotelAttendanceService {
    ...
    async clockIn(userId: string, companyId: string, shiftType: ShiftType): Promise<StaffAttendance> {
        ...
        if (openSession) {
            throw new ConflictException('User already has an open session. Please clock out first.');
        }
    ...
    async clockOut(userId: string, companyId: string): Promise<StaffAttendance> {
        ...
        if (!openSession) {
            throw new NotFoundException('No active attendance session found to clock out.');
        }
    ...
    async getDailyAttendance(companyId: string, date: string): Promise<StaffAttendance[]> {
    ...
    async getUserHistory(userId: string, companyId: string): Promise<StaffAttendance[]> {
        return this.attendanceRepository.find({
            where: {
                user: { id: userId },
                companyId
