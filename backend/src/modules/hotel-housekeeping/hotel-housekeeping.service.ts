...
export class HousekeepingService {
  ...
  async registerMinibarConsumption(
    companyId: string,
    dto: RegisterConsumptionDto,
  ): Promise<MinibarConsumption> {
    return this.dataSource.transaction(async (manager) => {
      // 1. Validate Room Status
      const room = await manager.findOne(HotelRoom, {
        where: { id: dto.roomId, companyId },
      });
      if (!room) throw new NotFoundException("Room not found");

      if (room.status !== RoomStatus.OCCUPIED) {
        throw new ConflictException(
          "Consumption can only be registered for OCCUPIED rooms.",
        );
      }
      ...
      if (!activeBooking) {
        throw new ConflictException(
          "No active CHECKED_IN booking found for this room.",
        );
      }
  ...
  async getPendingCleaningRoomsByCompany(
    companyId: string,
  ): Promise<HotelRoom[]> {
