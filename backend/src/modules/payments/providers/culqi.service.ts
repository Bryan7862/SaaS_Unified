...
export class CulqiService implements OnModuleInit {
  ...
  onModuleInit() {
    this.secretKey =
      this.configService.get<string>("CULQI_SECRET_KEY") ||
      "sk_test_placeholder";
    this.publicKey =
      this.configService.get<string>("CULQI_PUBLIC_KEY") ||
      "pk_test_placeholder";

    if (this.secretKey === "sk_test_placeholder") {
      this.logger.warn(
        "⚠️ Keys not set. Activando MOCK MODE. Los pagos serán simulados.",
      );
  ...
  async createOrder(
    ...
    try {
      ...
      this.logger.error(
        `Create Order Error: ${error.response?.data || error.message}`,
      );
  ...
  async getCharge(chargeId: string) {
    ...
    try {
      ...
      this.logger.error(
        `Get Charge Error: ${error.response?.data || error.message}`,
      );
