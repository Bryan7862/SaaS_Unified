...
export class AuthService {
  ...
  async register(registerDto: RegisterDto): Promise<User> {
    ...
    const savedCompany = await this.organizationsService.create(null, {
      name: companyName,
    });
  ...
  async createUser(
    ...
    const { email, password, defaultCompanyId, firstName, lastName } =
      createUserDto;
    ...
    if (defaultCompanyId) {
      ...
      if (
        ...
        throw new BadRequestException(
          "La suscripción de la organización no está activa. Por favor actualice su plan.",
        );
      }
      ...
      if (
        ...
        throw new BadRequestException(
          "Su periodo de prueba ha finalizado. Por favor suscríbase a un plan para agregar más usuarios.",
        );
      }

      // 4. Check Limits
      const currentUsersCount =
        await this.iamService.countCompanyUsers(defaultCompanyId);
      const { maxUsers } = this.subscriptionsService.getPlanLimits(
      ...
      if (currentUsersCount >= maxUsers) {
        throw new BadRequestException(
          `Has alcanzado el límite de usuarios de tu plan (${maxUsers}). Actualiza tu plan para agregar más.`,
        );
      }
    }
  ...
  async getUsers(
    ...
    if (!membership) {
      throw new BadRequestException(
        "Access Denied: You are not a member of this organization context or your account is suspended",
      );
    }
  ...
  async suspendUser(
    ...
    if (companyId) {
      ...
      const targetRole = await this.iamService.findUserRoleInCompany(
        userId,
        companyId,
      );
      if (!targetRole) {
      ...
      // Check Actor Role
      const actorRole = await this.iamService.findUserRoleInCompany(
        performedByUserId,
        companyId,
      );

      // Check Hierarchy: Only OWNER can touch OWNER
      if (targetRole.role.code === "OWNER") {
        // Assuming 'role' relation loaded
        if (!actorRole || actorRole.role.code !== "OWNER") {
          throw new BadRequestException(
            "Solo un PROPIETARIO puede suspender a otro PROPIETARIO.",
          );
        }
      }
    }
  ...
  async hardDeleteUser(userId: string): Promise<void> {
    ...
    if (user.defaultCompanyId) {
      const isOwner = user.userRoles.some(
        (ur) =>
          ur.role.code === "OWNER" && ur.companyId === user.defaultCompanyId,
      );
      if (isOwner) {
        const ownersCount = await this.iamService.countCompanyOwners(
          user.defaultCompanyId,
        );
        if (ownersCount <= 1) {
          throw new BadRequestException(
            "Cannot delete the last OWNER of an organization. Transfer ownership or delete the organization first.",
          );
        }
      }
    }
