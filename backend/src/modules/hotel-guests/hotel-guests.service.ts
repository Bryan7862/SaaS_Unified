...
export class HotelGuestsService {
  ...
  async findGuestByDocumentNumber(
    companyId: string,
  ...
  async updateGuestContactInfo(
    ...
    if (!guest)
      throw new NotFoundException("Guest not found within this organization.");
  ...
  private async checkDuplicateDocument(
    ...
    const existing = await this.findGuestByDocumentNumber(
      companyId,
      documentNumber,
    );
    if (existing) {
      throw new ConflictException(
        `Guest with document ${documentNumber} already exists in this organization.`,
      );
    }
  }
}
