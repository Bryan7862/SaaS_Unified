...
export class HotelRoomsService {
  ...
  async createFloorForOrganization(
    companyId: string,
    dto: CreateFloorDto,
  ): Promise<HotelFloor> {
  ...
  async deleteFloor(companyId: string, floorId: string): Promise<void> {
    ...
    if (floor.rooms && floor.rooms.length > 0) {
      throw new BadRequestException(
        "No se puede eliminar un piso que tiene habitaciones asignadas.",
      );
    }
  ...
  async deleteRoom(companyId: string, roomId: string): Promise<void> {
    ...
    if (activeBookingsCount > 0) {
      throw new ConflictException(
        "No se puede eliminar una habitación con reservas activas o futuras.",
      );
    }
  ...
  async createNewRoomForOrganization(
    companyId: string,
    dto: CreateRoomDto,
  ): Promise<HotelRoom> {
    ...
    if (existingRoom) {
      throw new ConflictException(
        `Room number ${dto.number} already exists in this organization.`,
      );
    }
    ...
    if (!floor) {
      throw new BadRequestException(
        "Invalid Floor ID. The floor must belong to your organization.",
      );
    }
    ...
    if (!category) {
      throw new BadRequestException(
        "Invalid Category ID. The category must belong to your organization.",
      );
    }
  ...
  async archiveFloor(
    ...
    const hasOccupiedRooms = floor.rooms.some(
      (room) => room.status === RoomStatus.OCCUPIED,
    );
    if (hasOccupiedRooms) {
      throw new ConflictException(
        "No se puede archivar el piso porque tiene habitaciones ocupadas.",
      );
    }
  ...
  async archiveRoom(
    ...
    if (room.status === RoomStatus.OCCUPIED) {
      throw new ConflictException(
        "No se puede archivar una habitación que está OCUPADA.",
      );
    }
