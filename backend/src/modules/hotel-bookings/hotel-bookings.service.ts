...
import { Repository, LessThanOrEqual, MoreThanOrEqual, And, Not } from 'typeorm';
...
export class HotelBookingsService {
    ...
    async createBooking(createBookingDto: CreateBookingDto, companyId: string): Promise<HotelBooking> {
        const { roomId, guestId, checkInDate, checkOutDate, totalAmount } = createBookingDto;
        ...
        if (!room) throw new NotFoundException('Room not found or does not belong to this organization.');

        const guest = await this.guestRepository.findOne({ where: { id: guestId, companyId } });
        if (!guest) throw new NotFoundException('Guest not found or does not belong to this organization.');

        // Availability Check
        await this.checkRoomAvailability(roomId, new Date(checkInDate), new Date(checkOutDate), companyId);
    ...
    async checkRoomAvailability(roomId: string, start: Date, end: Date, companyId: string): Promise<void> {
        ...
        const conflictingBooking = await this.bookingRepository.createQueryBuilder('booking')
            ...
            .andWhere('booking.status IN (:...statuses)', { statuses: [BookingStatus.PENDING, BookingStatus.CONFIRMED, BookingStatus.CHECKED_IN] })
            .andWhere('(booking.checkInDate < :end AND booking.checkOutDate > :start)', { start, end })
            .getOne();
    ...
    async processCheckIn(bookingId: string, companyId: string): Promise<HotelBooking> {
    ...
    async processCheckOut(bookingId: string, companyId: string): Promise<HotelBooking> {
        const booking = await this.bookingRepository.findOne({
            where: { id: bookingId, companyId },
            relations: ['room']
        });
        ...
        if (booking.status !== BookingStatus.CHECKED_IN) {
            throw new BadRequestException(`Cannot check-out booking with status ${booking.status}`);
        }
    ...
    async findActiveBookingsByRoom(roomId: string, companyId: string): Promise<HotelBooking[]> {
        return this.bookingRepository.find({
            where: {
                room: { id: roomId },
                companyId,
