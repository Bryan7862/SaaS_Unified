...
export class SubscriptionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const companyId =
      request.headers["x-company-id"] || request.user?.defaultCompanyId;
    ...
    if (!companyId) {
      ...
      throw new ForbiddenException(
        "Organization Context missing for Subscription Check",
      );
    }
    ...
    if (!ALLOWED_STATUSES.includes(subscription.status)) {
      throw new ForbiddenException(
        `Subscription is ${subscription.status}. Please update your plan.`,
      );
    }
