...
export class SubscriptionsService {
  ...
  async getCurrentSubscription(organizationId: string): Promise<Subscription> {
    ...
    if (!sub) {
      ...
      console.warn(
        `No subscription found for org ${organizationId}. creating default.`,
      );
  ...
  async suspendSubscription(
    ...
    if (!sub) {
      console.warn(
        `[Suspend] Subscription not found for Provider ID: ${providerSubscriptionId}`,
      );
  ...
  async cancelSubscription(
    ...
    if (!sub) {
      console.warn(
        `[Cancel] Subscription not found for Provider ID: ${providerSubscriptionId}`,
      );
      throw new NotFoundException("Subscription not found via Provider ID");
    }
