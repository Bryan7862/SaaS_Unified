...
export class OrganizationsService {
  ...
  async create(
    ...
    if (userId) {
      ...
      if (count >= MAX_ORGANIZATIONS_PER_USER) {
        throw new BadRequestException(
          `Has alcanzado el número máximo de organizaciones permitidas (${MAX_ORGANIZATIONS_PER_USER}).`,
        );
      }
    }
    ...
    if (userId) {
      ...
      if (ownerRole) {
        ...
        console.error(
          "OWNER role code not found. Organization created but user not assigned as Owner.",
        );
      }
    }
  ...
  async suspendOrganization(
    ...
    if (company.status === CompanyStatus.SUSPENDED) {
      throw new BadRequestException(
        "La organización ya se encuentra suspendida",
      );
    }
